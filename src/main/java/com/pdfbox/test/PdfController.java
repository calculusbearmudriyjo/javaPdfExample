...
public class PdfController {
    ...
    public void createPdfBox() throws IOException, COSVisitorException {
        createPdf();
    }

    @RequestMapping("/createOpenPdf")
    public void createOpenPdf() {
        Document document = new Document(PageSize.A4);

        try {

            // step 2:
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("/tmp/listboxes.pdf"));

            // step 3: we open the document
            document.open();

            com.lowagie.text.Table table = new com.lowagie.text.Table(2, 2); // 2 rows, 2 columns
            document.add(table);
            Paragraph paragraph = new Paragraph("converted to PdfPTable:");
            paragraph.setSpacingAfter(1);
            document.add(paragraph);
            table.setOffset(1);
            table.setPadding(1);
            IntStream.range(0, 20).forEach( i -> table.addCell("" + i));
